...
class SleepingBag : Runnable {
    override fun run() {
        ...
        MineTweakerAPI.recipes.addShaped(ItemBracketHandler.getItem("sleepingbag:sleepingBag", 0), arrayOf(arrayOf<IIngredient>(item, item, item), arrayOf<IIngredient>(ore, ore, ore), arrayOf<IIngredient>(item2, item2, item2)), null)
