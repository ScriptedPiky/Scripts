...
class HoloInventory : Runnable {
    override fun run() {
        ...
        IngredientTooltips.addTooltip(ItemBracketHandler.getItem("holoinventory:Hologlasses", 0), ExpandString.asFormattedText("<Hold Shift>"))
        IngredientTooltips.addShiftTooltip(ItemBracketHandler.getItem("holoinventory:Hologlasses", 0), ExpandString.add("Info:", MineTweakerAPI.format.red(ExpandString.asFormattedText("Adds Holographic inventory screen for all blocks with an inventory"))))
    }
}